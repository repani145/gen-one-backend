...
def validate_ALLOWED_ONLY_IF(json_spec, source_file, target_files, rule_name):
    errors = []
    error_set = set()
    ...
    for t in json_spec["target"]:
        ...
        with pd.ExcelFile(tgt_file) as tgt_xl:
            tgt_df = pd.read_excel(tgt_xl, sheet_name=t["tab"])
            tgt_df.index = tgt_df[t["primary_field"]]  # set index
    ...
    for idx, row in source_df.iterrows():
        ...
        if not all(is_valid) and not (pd.isna(source_value) or source_value == ""):
...
# rule_name = "material_plant_data-SOBSL should have value only if material_basic_data - MTART=[ZSEM] AND WERKS=[2400]"

# errors = validate_ALLOWED_ONLY_IF(json_spec, source_file, target_files,rule_name)
