...
def validate_NOT_ALLOWED_TYPE_CONDITION(json_spec, source_file, target_files, rule_name):
    """
    ...
    with pd.ExcelFile(source_file) as xl:
        source_df = pd.read_excel(xl, sheet_name=source_tab)

    # Load targets safely
    target_maps = []
    for t in json_spec["target"]:
        tgt_file = target_files[t["spec"]]
        with pd.ExcelFile(tgt_file) as xl_tgt:
            tgt_df = pd.read_excel(xl_tgt, sheet_name=t["tab"])
            target_maps.append({"spec": t, "df": tgt_df.set_index(t["primary_field"])})
    ...
    for idx, row in source_df.iterrows():
        ...
        for t in target_maps:
            ...
            # --- Primary exists, check field ---
            tgt_val = tgt_df.loc[primary_value, spec["field"]]
            op = spec["operator"]
            values = spec["values"]
        ...
        # If all target checks pass, mark as error for NOT_ALLOWED_TYPE
        if all(is_valid):
...
# source_file = "D:/Learn/Py/files/Routings.xlsx"
# target_files = {"material1": "D:/Learn/Py/files/Material_Data_Dallas_23June2025.xlsx"}
# rule_name = "routing-MATNR should not have type material_basic_data- MTART=[ZRAW,ZHAW]"

# errors = validate_NOT_ALLOWED_TYPE_CONDITION(json_spec, source_file, target_files,rule_name)
